...
public struct Endpoint<A> {
    ...
    public func compactMap<B>(_ transform: @escaping (A) -> Result<B, Error>) -> Endpoint<B> {
        Endpoint<B>(request: request, expectedStatusCode: expectedStatusCode, parse: { data, response in
...
public extension Endpoint where A == () {
    ...
    ///   - timeOutInterval: the timeout interval for his request
    ///   - query: query parameters to append to the url
    init(_ method: Method, url: URL, accept: ContentType? = nil, contentType: ContentType? = nil, body: Data? = nil, headers: [String: String] = [:], expectedStatusCode: @escaping (Int) -> Bool = expected200to300, timeOutInterval: TimeInterval = 10, query: [String: String] = [:]) {
    ...
    init<B: Encodable>(json method: Method, url: URL, accept: ContentType? = .json, body: B, headers: [String: String] = [:], expectedStatusCode: @escaping (Int) -> Bool = expected200to300, timeOutInterval: TimeInterval = 10, query: [String: String] = [:], encoder: JSONEncoder = JSONEncoder()) {
...
public extension Endpoint where A: Decodable {
    ...
    ///   - decoder: the decoder that's used for decoding `A`s.
    ///   - encoder: the encoder that's used for encoding `A`s.
    init<B: Encodable>(json method: Method, url: URL, accept: ContentType = .json, body: B? = nil, headers: [String: String] = [:], expectedStatusCode: @escaping (Int) -> Bool = expected200to300, timeOutInterval: TimeInterval = 10, query: [String: String] = [:], decoder: JSONDecoder = JSONDecoder(), encoder: JSONEncoder = JSONEncoder()) {
...
public extension URLSession {
    func load<A>(_ e: Endpoint<A>) async throws -> A {
        ...
        do {
            ...
            guard e.expectedStatusCode(h.statusCode) else {
                let statusCodeError = WrongStatusCodeError(statusCode: h.statusCode, response: h, responseBody: data)
                throw HTTPError.serverSide(statusCodeError)
            }
            return try e.parse(data, resp).get()
